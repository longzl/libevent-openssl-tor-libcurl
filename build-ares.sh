...
if [ ! -e "${SRCDIR}/c-ares-${VERSION}.tar.gz" ]; then
	echo "Downloading c-ares-${VERSION}.tar.gz"
	curl -LO http://c-ares.haxx.se/download/c-ares-${VERSION}.tar.gz
fi
echo "Using c-ares-${VERSION}.tar.gz"

tar zxf c-ares-${VERSION}.tar.gz -C $SRCDIR
cd "${SRCDIR}/c-ares-${VERSION}"
...
do
	...
	./configure --disable-shared --enable-static $EXTRA_CONFIG --prefix="${INTERDIR}/${PLATFORM}${SDKVERSION}-${ARCH}.sdk"
